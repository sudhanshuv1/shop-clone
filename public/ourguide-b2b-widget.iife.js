...
`;window.ourguide=(e,n)=>{e==="registerTools"&&n&&typeof n=="object"&&rn(n)},(function(){const e=document.currentScript??document.querySelector('script[data-product-id][src*="ourguide-b2b-widget"]');if(!e){console.error("[Ourguide-B2B] Could not find script element");return}const n=e.dataset.productId||"",t=e.dataset.apiUrl??"http://localhost:3000",o=e.dataset.position??"bottom-right";if(!n){console.error("[Ourguide-B2B] data-product-id is required");return}async function s(l,c){try{if(l==="identify"){const i=c??{};if(!i.token||typeof i.token!="string"||!i.token.trim()){console.warn("[Ourguide-B2B] identify: token is required");return}const p=ie(n);await tn(t,n,p,i.token,i.name);return}if(l==="resetUser"){const i=ie(n);await on(t,n,i),gn(n);return}}catch(i){const p=i instanceof Error?i.message:"Unknown error";console.warn(`[Ourguide-B2B] ${l} failed: ${p}`)}}const r=window,a=Array.isArray(r.ourguide)?r.ourguide:[];r.ourguide=(l,c)=>{s(l,c)};for(const l of a)Array.isArray(l)&&(l[0]==="identify"||l[0]==="resetUser")&&s(l[0],l[1]);const _=document.createElement("style");_.textContent=wn,document.head.appendChild(_);const u=document.createElement("div");u.id="og2-widget-root",document.body.appendChild(u),Ve(pe(vn,{productId:n,apiUrl:t,position:o}),u)})()})();
